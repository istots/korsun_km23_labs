...
public class MyContainer implements Container{
    ...
    private static<T> Constructor<T> findConstructor(Class<T> clazz) {
     ...
     // Додаткова функція для визначення синглтонів
     public <T> void bindSingleton(Class<T> clazz) {
        implementationClasses.put(clazz, null);
        singletons.put(clazz, null);
    }

    // Додаткова функція для налаштування ін'єкції залежностей для конструкторів без анотації @Inject
     public <T> void bindDefaultInjection(Class<T> clazz) {
        implementationClasses.put(clazz, null);
    }
    
    // Додаткова функція для явного вказівки класу як синглтона
    public <T> void bindSingletonClass(Class<T> clazz) {
        implementationClasses.put(clazz, null);
        singletons.put(clazz, clazz);
    }
}
